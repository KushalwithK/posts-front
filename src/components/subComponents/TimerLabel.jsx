...
const TimerLabel = ({ timerInfo, setTimerInfo, currentTimerInfo }) => {

    const { decryptPassword } = useContext(AppContext);
    const [thisTimerInfo, setThisTimerInfo] = useState(currentTimerInfo)
    const [transformedTime, setTransformedTime] = useState(calculateTimeDuration(thisTimerInfo.timeElapsed))
    ...
    const handleTimerStart = () => {
        if (!thisTimerInfo.start) {
            ...
            intervalRef.current = setInterval(() => {
                setThisTimerInfo(old => {
                    return {
                        ...old, timeElapsed: old.timeElapsed + 1
                    }
                })
            }, 1000)
        }
    ...
    return (
        <Main>
            ...
            <p>{`${transformedTime.days > 0 ? transformedTime.days + "d" : ""} ${transformedTime.hours > 0 ? transformedTime.hours + "h" : ""} ${transformedTime.minutes > 0 ? transformedTime.minutes + "m" : ""} ${transformedTime.seconds}s`}</p>
